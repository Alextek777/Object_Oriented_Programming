    ...
    : QTextEdit(parent)
{
    ...
    setWindowIcon(QPixmap(":/img/imgFiles/document.png"));
    setWindowTitle("[*]");
...
{
    QFile file(fileName);
    if(QFileInfo(fileName).completeSuffix() == "cpp" || QFileInfo(fileName).completeSuffix() == "h" || QFileInfo(fileName).completeSuffix() == "hpp")
    {
        highlighter = new CustomHighlighter("C++",this);
